...
// calendar - time picking should be better, , review going back buttons, fix recurring trips (they work if the interval is bigger than the length of the trip sheck if that is correct; they are recucring faster (every minute not the set value))
// pagination buttons should be invisible, tripservice does it every minute not when it should possible recurenceinterval is null
